...
| Ch 10 — Putting DSPy into Production | `chapter10/` — 3 notebooks (MLflow tracking, FastAPI invoice API, Gradio DSPyUI) |
| Ch 11 — Optimizing Coding Agents with DSPy | `chapter11/` — 5 notebooks (landing-page skill optimizer, image-CLI optimizer, RLM skill discovery, AGENTS.md testing, Clawsona persona) |
...
You do not need to create or activate a virtual environment by hand. `uv sync`
creates `.venv` and installs the exact versions in `uv.lock`.

```bash
# 1. Install uv (macOS/Linux; skip if it is already installed)
...
# 3. Create .venv and install the locked environment
uv sync --frozen

# 4. Create your local API-key file, then edit it
cp .env.example .env

# 5. Verify the installation and required OpenAI key
uv run python scripts/check_setup.py --require-openai-key

# 6. Start Jupyter and open chapter01/hello-dspy.ipynb
uv run jupyter lab
```

On Windows PowerShell, install uv and copy the environment template with:
...
If you cannot use uv, create a Python 3.12–3.14 virtual environment and install
the generated compatibility requirements:
...
Do not install a separate DSPy version on top of this environment; the examples
and saved artifacts are validated against the version declared by the repo.
...
| `ANTHROPIC_API_KEY` | Provider-switch demos (Ch 2), Anthropic prompt-cache examples in Ch 10, and several Ch 11 notebooks |
| `OPENROUTER_API_KEY` | Ch 11 §11.3.3 Clawsona notebook (calls `openrouter/anthropic/claude-opus-4.7`) |
| `GEMINI_API_KEY` | Optional — Gemini provider-switch demos |
| `FAL_KEY` | Ch 9 §9.5 (fashion video generator) only |
| `SERPER_API_KEY` | Ch 8 web search; Ch 9 §9.4 news researcher |
| `TAVILY_API_KEY` | Optional Ch 8 web-search provider |
| `QDRANT_API_KEY` | Optional Ch 8 remote Qdrant service |
...
|---|---|---|
| MLflow | Ch 3 (optional tracing), Ch 10 §10.1 | `mlflow server --backend-store-uri sqlite:///mydb.sqlite --host 127.0.0.1 --port 5000` |
...
For a non-notebook example, use its real repository path with `uv run`, such as:

```bash
uv run python chapter08/mcp_server.py
uv run python chapter06/run_live_optimizer.py --help
```

You can activate `.venv` if you prefer, but it is not required: `uv run`
executes each command in the project environment directly.
...
| Ch 10 | $0.30–1.00 |
| Ch 11 | $1–3 with smoke caps; full GEPA runs can run $10+ per notebook if opted in |
