...
Transformers/TRL. The optimization flow remains DSPy-native; the small
BootstrapFinetune subclass only validates accepted-label balance before DSPy
formats and submits the training data.
"""
...
class AppleSiliconProvider(Provider):
    ...
    def finetune(
        ...
        try:
            saved_adapter_path = str(adapter_dir.relative_to(REPO_ROOT))
        except ValueError:
            saved_adapter_path = str(adapter_dir)
        summary = {
